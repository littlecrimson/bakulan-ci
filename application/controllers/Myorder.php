...
class Myorder extends MY_Controller {
    ...
    public function index()
    {
        ...
        $data['content']    = $this->myorder->where('id_user',$this->id)->orderBy('date','DESC')->all();
    ...
    {
        $data['order']	= $this->myorder->where('invoice', $invoice)->single();
		if (!$data['order']) {
		...
		if ($data['order']->status !== 'waiting') {
			...
			$data['order_confirm']	= $this->myorder->where('id_order', $data['order']->id)->single();
        }
