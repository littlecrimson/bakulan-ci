...
class Profile extends MY_Controller {
    ...
    {
        $data['content'] = $this->profile->where('id',$this->id)->single();
        if(!$data['content']){
        ...
        if(!empty($_FILES) && $_FILES['image']['name'] !== ''){
            ...
            if($upload){
                if($data['input']->image !== ''){
                    unlink('./images/user/'.$data['content']->image);
                    $this->profile->uploadImage('image',$imageName);
                }
        ...
        if ($this->profile->where('id', $this->id)->update($data['input'])) {
    ...
    public function unique_email(){
        ...
        $user = $this->profile->where('email',$email)->single();
